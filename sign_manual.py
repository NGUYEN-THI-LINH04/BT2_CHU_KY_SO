...
# === CONFIG ===
BASE = r"D:\PDF_SIGN_PROJECT_"
pdf_in = os.path.join(BASE, "BTVN2.pdf")
pdf_out = os.path.join(BASE, "output", "BTVN2_signed.pdf")
key_file = os.path.join(BASE, "certs", "signer_key.pem")
cert_file = os.path.join(BASE, "certs", "signer_cert.pem")
root_file = os.path.join(BASE, "certs", "rootCA_cert.pem")
sign_img = os.path.join(BASE, "chuky.jpg")
appearance_img = os.path.join(BASE, "appearance.jpg")

DEFAULT_PHONE = "0383792082"
...
unsigned_pdf = os.path.join(BASE, "output", "temp_unsigned.pdf")
os.makedirs(os.path.dirname(unsigned_pdf), exist_ok=True)
...
pkcs7 = (
    ...
    .add_certificate(cert)         # thêm dòng này
...
print("✅ PDF ký thành công (ByteRange & Contents chuẩn).")
print("📄 Output:", pdf_out)
